...
def coffate_fn(examples):
    texts, all_labels = [], []
    for text, ner_labels in examples:
        texts.append(text)
    ...
    labels_input = torch.tensor(targets)
...
for epoch in range(EPOCHS):
    ...
    for batch in tqdm(train_loader, desc=f'Epoch {epoch+1}'):
        ...
        inputs, labels_input = [x.to(device) for x in batch]
        ...
        labels =labels_input.view(-1)

        # Compute the loss
        loss = loss_fn(outputs.logits.view(-1, 5), labels)
        loss.backward()
        optimizer.step()
    ...
    precision = precision_score(val_true_labels, val_pred_labels, average='macro', zero_division=1)
